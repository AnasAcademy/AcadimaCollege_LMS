            ...
            @foreach($bundle->bundleWebinars as $bundleWebinar)
                ...
                @endphp
                    ...
                    <div class="col-12 col-lg-6 mb-4">
                        <div class="webinar-card webinar-list d-flex bg-secondary-acadima shadow border">
                            <div class="image-box">
                                ...
                                @switch($webinar->status)
                                    ...
                                    @if($webinar->isWebinar())
                                        @if($webinar->start_date > time())
                                            <span class="badge badge-primary text-light">{{  trans('panel.not_conducted') }}</span>
                                        @elseif($webinar->isProgressing())
                                            <span class="badge badge-secondary text-light">{{ trans('webinars.in_progress') }}</span>
                                        @else
                                            <span class="badge badge-secondary text-light">{{ trans('public.finished') }}</span>
                                        @endif
                                    @else
                                        <span class="badge badge-secondary text-light">{{ trans('webinars.'.$webinar->type) }}</span>
                                    @endif
                                    @break
                                    ...
                                    <span class="badge badge-warning text-dark-blue">{{ trans('public.waiting') }}</span>
                                    @break
                                ...
                                @if($webinar->isWebinar())
                                    <div class="progress bg-greenblue">
                            ...
                            <div class="webinar-card-body w-100 d-flex flex-column">
                                    ...
                                    {{-- @if($authUser->id == $webinar->creator_id or $authUser->id == $webinar->teacher_id) --}}
                                        <div class="btn-group dropdown table-actions">
                                            ...
                                            <div class="dropdown-menu ">
                                                ...
                                                <a href="{{ $bundleWebinar->getWebinarLearningPageUrl() }}" target="_blank" class="webinar-actions d-block mt-10">
                                ...
                                <div class="d-flex align-items-center justify-content-between flex-wrap mt-auto">
                                    <div class="d-flex align-items-start flex-column mt-20 mr-15">
                                        ...
                                        <span class="stat-value text-black">{{ $webinar->id }}</span>
                                    </div>

                                    <div class="d-flex align-items-start flex-column mt-20 mr-15">
                                        <span class="stat-title">{{ trans('public.category') }}:</span>
                                        <span class="stat-value text-black">{{ !empty($webinar->category_id) ? $webinar->category->title : '' }}</span>
                                    </div>

                                    @if($webinar->isProgressing() and !empty($nextSession))
                                        <div class="d-flex align-items-start flex-column mt-20 mr-15">
                                            <span class="stat-title">{{ trans('webinars.next_session_duration') }}:</span>
                                            <span class="stat-value text-black">{{ convertMinutesToHourAndMinute($nextSession->duration) }} Hrs</span>
                                        </div>

                                        @if($webinar->isWebinar())
                                            <div class="d-flex align-items-start flex-column mt-20 mr-15">
                                                <span class="stat-title">{{ trans('webinars.next_session_start_date') }}:</span>
                                                <span class="stat-value text-black">{{ dateTimeFormat($nextSession->date,'j M Y') }}</span>
                                            </div>
                                        @endif
                                    @else
                                        <div class="d-flex align-items-start flex-column mt-20 mr-15">
                                            <span class="stat-title">{{ trans('public.duration') }}:</span>
                                            <span class="stat-value text-black">{{ $webinar->duration }} Hrs</span>
                                        </div>

                                        @if($webinar->isWebinar())
                                            <div class="d-flex align-items-start flex-column mt-20 mr-15">
                                                <span class="stat-title">{{ trans('public.start_date') }}:</span>
                                                <span class="stat-value text-black">{{ dateTimeFormat($webinar->start_date,'j M Y') }}</span>
                                            </div>
                                        @endif
                                    ...
                                    @if($webinar->isTextCourse() or $webinar->isCourse())
                                        <div class="d-flex align-items-start flex-column mt-20 mr-15">
                                            ...
                                            <span class="stat-value text-black">{{ $webinar->files->count() }}</span>
                                        </div>
                                    @endif
                                    ...
                                    @if($webinar->isTextCourse())
                                        <div class="d-flex align-items-start flex-column mt-20 mr-15">
                                            ...
                                            <span class="stat-value text-black">{{ $webinar->textLessons->count() }}</span>
                                        </div>
                                    @endif
                                    ...
                                    @if($webinar->isCourse())
                                        <div class="d-flex align-items-start flex-column mt-20 mr-15">
                                            ...
                                            <span class="stat-value text-black">{{ ($webinar->downloadable) ? trans('public.yes') : trans('public.no') }}</span>
                                        </div>
                                    @endif

                                    {{-- <div class="d-flex align-items-start flex-column mt-20 mr-15">
                                        <span class="stat-title">{{ trans('panel.sales') }}:</span>
                                        <span class="stat-value text-black">{{ count($webinar->sales) }} ({{ (!empty($webinar->sales) and count($webinar->sales)) ? handlePrice($webinar->sales->sum('amount')) : 0 }})</span>
                                    </div> --}}
                                    ...
                                    @if(!empty($webinar->partner_instructor) and $webinar->partner_instructor and $authUser->id != $webinar->teacher_id and $authUser->id != $webinar->creator_id)
                                        <div class="d-flex align-items-start flex-column mt-20 mr-15">
                                            ...
                                            <span class="stat-value ttext-black">{{ $webinar->teacher->full_name }}</span>
                                        </div>
                                    @elseif($authUser->id != $webinar->teacher_id and $authUser->id != $webinar->creator_id)
                                        <div class="d-flex align-items-start flex-column mt-20 mr-15">
                                            ...
                                            <span class="stat-value text-black">{{ $webinar->teacher->full_name }}</span>
                                        </div>
                                    @elseif($authUser->id == $webinar->teacher_id and $authUser->id != $webinar->creator_id and $webinar->creator->isOrganization())
                                        <div class="d-flex align-items-start flex-column mt-20 mr-15">
                                            ...
                                            <span class="stat-value text-black">{{ $webinar->creator->full_name }}</span>
                                        </div>
                                    @endif
                                </div>
                            </div>
